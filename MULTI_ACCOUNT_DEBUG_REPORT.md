...
**Evidence**:
```
Found target account ***REMOVED*** from Protection Group pg-current-account
Using target account ***REMOVED*** with default role name
Result: {
  "AccountId": "***REMOVED***",
...
def determine_target_account_context(plan: Dict) -> Dict:
    try:
        ...
        current_account_id = os.environ.get('AWS_ACCOUNT_ID', '***REMOVED***')
