...
When creating a Protection Group and selecting account "DEMO_TARGET (160885257264)", the server discovery panel was showing EC2 instances from the **orchestration account (438465159935)** instead of the **target account (160885257264)**.

### User Experience
```
Create Protection Group
Region: us-west-2 (Oregon)
Account: DEMO_TARGET - 160885257264

Server Selection:
❌ Showing: hrp-core-app01-az1 (from orchestration account 438465159935)
✅ Expected: Servers from target account 160885257264
```
...
```json
{
  ...
  "roleArn": "arn:aws:iam::160885257264:role/DRSOrchestrationRole-dev"
...
```
Found target account 160885257264 with role None
Creating DRS client for current account in region us-west-2
```

This caused the DRS client to query the orchestration account (438465159935) instead of the target account (160885257264).
...
2. **If not found**, extract from `roleArn`:
   - Input: `arn:aws:iam::160885257264:role/DRSOrchestrationRole-dev`
   - Split by `/`: `["arn:aws:iam::160885257264:role", "DRSOrchestrationRole-dev"]`
...
3. **Select Account**: "DEMO_TARGET - 160885257264"
4. **Select Region**: "us-west-2 (Oregon)"
5. **Go to "Server Selection" tab**
6. **Verify**: Should show servers from target account 160885257264 only
...
**Expected Log Output**:
```
Found target account 160885257264 with role DRSOrchestrationRole-dev
Creating cross-account DRS client for account 160885257264 using role DRSOrchestrationRole-dev
Assuming role: arn:aws:iam::160885257264:role/DRSOrchestrationRole-dev
Successfully created cross-account DRS client for account 160885257264
```
...
```
GET /drs/source-servers?region=us-west-2&accountId=160885257264

Response should contain:
- Servers from target account 160885257264
- No servers from orchestration account 438465159935
```
...
- Successfully assumes role: `arn:aws:iam::160885257264:role/DRSOrchestrationRole-dev`
...
- **Orchestration Account**: 438465159935 (where Lambda functions run)
- **Target Account**: 160885257264 (recovery destination) - labeled as "DEMO_TARGET"
- **Staging Account**: 664418995426 (extended source servers)
- **Networking Account**: 615299763609 (shared VPC via AWS RAM)

### Cross-Account IAM Role

**Role Name**: `DRSOrchestrationRole-dev`  
**Full ARN**: `arn:aws:iam::160885257264:role/DRSOrchestrationRole-dev`  
**External ID**: `drs-orchestration-dev-160885257264`
...
```json
{
  ...
  "Statement": [{
    ...
    "Condition": {
      "StringEquals": {
        "sts:ExternalId": "drs-orchestration-dev-160885257264"
      }
    }
  }]
}
