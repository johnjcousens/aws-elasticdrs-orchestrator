...
└── .amazonq/rules/               # Amazon Q project rules and memory bank
```
...
- **api-stack.yaml**: API Gateway, Cognito User Pool, Step Functions state machine
...
- **step-functions-stack.yaml**: Step Functions orchestration state machine
...
│   ├── pages/               # Page-level components (Dashboard, ProtectionGroups, RecoveryPlans, Executions)
...
- **index.py** → `api-handler`: REST API endpoints for protection groups, recovery plans, executions, DRS integration
- **orchestration_stepfunctions.py** → `orchestration-stepfunctions`: Step Functions orchestration engine with wave execution and DRS job polling
...
- Step Functions state machine coordinates wave execution
- Lambda functions poll DRS job status asynchronously
- DynamoDB stores execution state for recovery and monitoring
- CloudWatch Logs capture detailed execution traces

### Frontend Architecture
- Single-page application with client-side routing
- CloudScape components for consistent AWS UI/UX
- Amplify Auth for Cognito integration
...
│   ├── api-stack.yaml
│   └── frontend-stack.yaml
├── lambda/                  # Lambda deployment packages (ZIP files)
│   ├── api-handler.zip
...
- **Recovery Plans** reference Protection Groups and define wave execution order
- **Executions** instantiate Recovery Plans with drill/recovery mode
- **Step Functions** orchestrate wave-by-wave execution with DRS job polling
