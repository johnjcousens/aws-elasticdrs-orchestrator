...
```bash
# Check stack status
aws cloudformation describe-stacks --stack-name aws-drs-orchestration-dev --region us-east-2

# Verify S3 has latest
aws s3 ls s3://aws-drs-orchestration/lambda/ --region us-east-2
```
...
- Project name: "aws-drs-orchestration" (matches sync script)
...
aws s3 ls s3://aws-drs-orchestration/lambda/ --region us-east-2
aws s3 ls s3://aws-drs-orchestration/cfn/ --region us-east-2
aws s3 ls s3://aws-drs-orchestration/frontend/ --region us-east-2
```
...
aws cloudformation create-stack \
  --stack-name aws-drs-orchestration-dev-TEST \
  --template-url https://s3.amazonaws.com/aws-drs-orchestration/cfn/master-template.yaml \
  --parameters \
