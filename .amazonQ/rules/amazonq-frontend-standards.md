...
<FormField label="Region">
  <Select
    ...
    options={[
      { value: 'us-east-2', label: 'US East (N. Virginia)' },
...
## CSS and Styling Rules (CRITICAL)

### 1. NO Inline Styles (MANDATORY)
```typescript
// ❌ NEVER use inline styles
<div style={{ padding: '16px', color: '#5f6b7a' }}>Content</div>
...
### 2. NO Hardcoded Values (MANDATORY)
```css
...
/* ✅ ALWAYS use CloudScape design tokens */
.container {
...
### 3. CSS Module Pattern (MANDATORY)
```typescript
// Component.tsx
import styles from './Component.module.css';
...
### 4. NEVER Override CloudScape Components (CRITICAL)
```typescript
// ❌ NEVER add className to CloudScape components (deprecated)
<Button className={styles.customButton}>Click</Button>

// ❌ NEVER override CloudScape internal styles
.customButton {
  background: red !important; /* Breaks on updates */
}

// ✅ ALWAYS wrap in styled div
<div className={styles.buttonWrapper}>
  <Button variant="primary">Click</Button>
</div>

// ✅ ALWAYS use CloudScape variant props
<Button variant="primary">Primary</Button>
<Button variant="normal">Normal</Button>
```

### 5. Z-Index Layering (MANDATORY)
```css
/* ALWAYS use centralized z-index scale from styles/z-index.css */
.dropdown {
...
.tooltip {
  z-index: var(--z-index-tooltip); /* 3000 */
}

/* ❌ NEVER use hardcoded z-index */
.element {
  z-index: 9999; /* WRONG */
}
```

### 6. camelCase Naming (MANDATORY)
```css
...
/* ❌ NEVER use kebab-case or snake_case */
.container-wrapper { } /* WRONG */
.header_title { } /* WRONG */
```

### 7. CloudScape Design Token Reference
```css
/* Colors */
var(--awsui-color-text-body-default)        /* Primary text */
var(--awsui-color-text-body-secondary)      /* Secondary text */
var(--awsui-color-text-link-default)        /* Links (#0972d3) */
var(--awsui-color-text-status-success)      /* Success (#037f0c) */
var(--awsui-color-text-status-error)        /* Error (#d13212) */
var(--awsui-color-background-container-content) /* Container bg */

/* Spacing (use scaled tokens) */
var(--awsui-space-scaled-xxs)  /* 4px */
var(--awsui-space-scaled-xs)   /* 8px */
var(--awsui-space-scaled-s)    /* 12px */
var(--awsui-space-scaled-m)    /* 16px */
var(--awsui-space-scaled-l)    /* 20px */
var(--awsui-space-scaled-xl)   /* 24px */
var(--awsui-space-scaled-xxl)  /* 32px */

/* Typography */
var(--awsui-font-size-body-s)      /* 12px */
var(--awsui-font-size-body-m)      /* 14px */
var(--awsui-font-size-heading-s)   /* 16px */
var(--awsui-font-size-heading-m)   /* 18px */
var(--awsui-font-weight-heading)   /* 600 */
var(--awsui-font-weight-bold)      /* 700 */

/* Borders */
var(--awsui-border-radius-container)  /* Rounded corners */
```

### 8. AWS Console Design Compliance
```css
/* ✅ Thin borders (1px) for containers */
.container {
  border: 1px solid var(--awsui-color-border-container-top);
}

/* ✅ Shadows only for interactive/transient elements */
.modal {
  box-shadow: var(--awsui-shadow-modal);
}

/* ❌ NO shadows on static containers */
.card {
  box-shadow: none; /* Static elements don't need shadows */
}
```

## Recommended CloudScape Components

### 1. Wizard Component (Multi-Step Workflows)
```typescript
// USE for Protection Group/Recovery Plan creation
<Wizard
  steps={[
    { title: 'Select Servers', content: <ServerSelection /> },on /> },
...
### 2. AttributeEditor Component (Key-Value Editing)
```typescript
// USE for tags, per-server overrides, static IPs
<AttributeEditor
  items={tags}
...
### 3. ExpandableSection Component (Progressive Disclosure)
```typescript
// USE for advanced options, optional fields
...
### 4. TokenGroup Component (Tag Display)
```typescript
// USE for selected servers, security groups, subnets
<TokenGroup
...
### 5. KeyValuePairs Component (Detail Display)
```typescript
// USE for server details, account info, metadata
<KeyValuePairs
  columns={3}
  items={[
...
### 6. Popover Component (Contextual Help)
```typescript
// USE for help text, warnings, status details
<Popover
...
### 7. Cards Component (Gallery View)
```typescript
// USE for protection groups, recovery plans, dashboards
<Cards
...
### 8. CodeEditor Component (JSON Editing)
```typescript
// USE for configuration export/import, user data scripts
<CodeEditor
  language="json"
