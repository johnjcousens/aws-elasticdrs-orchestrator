...
### 2. External Orchestration Integration Mode
The CloudFormation stack uses an externally-created orchestration role (provided externally).

**When to use**: Integrating with the External Orchestration Platform orchestration engine.

**CloudFormation Parameter**: `OrchestrationRoleArn` = "arn:aws:iam::ACCOUNT:role/ExternalOrchestrationRole"

**Requirements for External Role**:
...
For External Orchestration integration, use this CloudFormation template to create the orchestration role:

```yaml
AWSTemplateFormatVersion: '2010-09-09'
Description: 'External Orchestration Role for DRS Orchestration Platform'
...
Outputs:
  OrchestrationRoleArn:
    Description: 'ARN of the External Orchestration Role'
    Value: !GetAtt ExternalOrchestrationRole.Arn
    Export:
      Name: !Sub '${AWS::StackName}-OrchestrationRoleArn'

  OrchestrationRoleName:
    Description: 'Name of the External Orchestration Role'
    Value: !Ref ExternalOrchestrationRole
    Export:
...
| 1.1 | January 16, 2026 | Added complete CloudFormation template for External Orchestration integration |
...
## External Orchestration Integration Checklist

When creating the orchestration role for External Orchestration integration, ensure:
...
For questions about the orchestration role or External Orchestration integration:
