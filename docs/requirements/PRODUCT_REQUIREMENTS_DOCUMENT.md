...
- **CloudFormation**: 16 templates (1 master + 15 nested stacks)
...
- Protection Group management (6 endpoints)
- Recovery Plan management (6 endpoints)
- Target account management (5 endpoints)
- Configuration updates (4 endpoints)
- Tag sync configuration
- 21 CRUD endpoints total

**execution-handler** (DR Operations):
- Execution lifecycle management (11 endpoints)
...
- List/describe operations (12 endpoints)
...
**frontend-deployer** (Frontend Build & Deployment):
- React frontend build (npm build)
- Deploy to S3 bucket
...
- **ExecutionDetailsPageMinimal**: Minimal execution view
...
| api-gateway-resources-stack.yaml | API paths | 50+ URL path resources |
...
| api-gateway-deployment-stack.yaml | API deployment | Stage deployment, throttling (100 req/min) |
| step-functions-stack.yaml | Orchestration | State machine with waitForTaskToken (1-year timeout) |
| eventbridge-stack.yaml | Event scheduling | Execution polling rules (1-minute intervals) |
| frontend-stack.yaml | Frontend hosting | S3 bucket, CloudFront distribution (conditional) |
| notification-stack.yaml | Notifications | 3 SNS topics, email subscriptions |
| cross-account-role-stack.yaml | Multi-account | Cross-account IAM roles |
| github-oidc-stack.yaml | CI/CD | GitHub Actions OIDC authentication (optional) |
...
- **v4.0**: Comprehensive codebase analysis update - corrected Lambda count (6), API endpoints (44), CloudFormation templates (16), component count (35+), added frontend-deployer documentation
