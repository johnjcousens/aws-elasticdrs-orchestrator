...
```
Source VM → Mobility Service → Process Server → Azure Storage → Target VM
           ↓
...
#### Base API Structure
```
https://management.azure.com/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.RecoveryServices/
├── vaults/                    # Recovery Services Vaults
├── replicationFabrics/        # Fabric management (sites)
├── replicationProtectionContainers/  # Protection containers
├── replicationProtectedItems/ # Protected VMs and servers
├── replicationRecoveryPlans/  # Recovery plans
├── replicationJobs/          # Job monitoring
└── replicationPolicies/      # Replication policies
```
