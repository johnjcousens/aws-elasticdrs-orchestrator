...
aws apigateway get-rest-apis --query 'items[?name==`aws-drs-orchestration-api-dev`]'

# Check CloudWatch Logs for API usage patterns
aws logs filter-log-events \
  --log-group-name /aws/apigateway/aws-drs-orchestration-api-dev \
...
aws cloudformation update-stack \
  ...
  --parameters \
    ...
    ParameterKey=DeploymentBucket,ParameterValue=aws-drs-orchestration-dev \
...
```json
{
  "Effect": "Allow",
  "Principal": {
    "AWS": "arn:aws:iam::YOUR_ACCOUNT_ID:role/aws-drs-orchestration-orchestration-role-dev"
  },
  "Action": "lambda:InvokeFunction",
  "Resource": "arn:aws:lambda:us-east-1:YOUR_ACCOUNT_ID:function:aws-drs-orchestration-query-handler-dev"
}
```
...
aws cloudformation update-stack \
  --stack-name aws-drs-orchestration-dev \
  --template-url https://s3.amazonaws.com/aws-drs-orchestration-dev/cfn/master-template.yaml \
  --parameters \
    ParameterKey=ProjectName,ParameterValue=aws-drs-orchestration \
    ParameterKey=Environment,ParameterValue=test \
...
const response = await lambda.invoke({
  FunctionName: 'aws-drs-orchestration-data-management-handler-dev',
  InvocationType: 'RequestResponse',
...
lambda:InvokeFunction on resource: arn:aws:lambda:us-east-1:123456789012:function:aws-drs-orchestration-query-handler-dev
```
...
# Retry stack update
aws cloudformation update-stack --stack-name aws-drs-orchestration-dev ...
```
...
#     "Effect": "Allow",
#     "Principal": {
#       "AWS": "arn:aws:iam::ORCHESTRATION_ACCOUNT:role/aws-drs-orchestration-orchestration-role-dev"
#     },
#     "Action": "sts:AssumeRole"
#   }]
# }
  ...
  "Statement": [
    {
      ...
      "Action": "lambda:InvokeFunction",
      "Resource": [
        "arn:aws:lambda:us-east-1:123456789012:function:aws-drs-orchestration-query-handler-dev",
        "arn:aws:lambda:us-east-1:123456789012:function:aws-drs-orchestration-execution-handler-dev"
      ]
    }
  ]
}
...
aws cloudwatch put-metric-alarm \
  ...
  --dimensions Name=FunctionName,Value=aws-drs-orchestration-execution-handler-dev
```
