...
- **Staging Account**: 664418995426 (creates encrypted EBS snapshots)
- **Target Account**: 160885257264 (needs to decrypt snapshots for recovery)
...
cat > kms-key-policy.json << 'EOF'
{
  ...
  "Statement": [
    ...
    },
    {
      ...
      "Principal": {
        "AWS": [
          "arn:aws:iam::160885257264:root",
          "arn:aws:iam::160885257264:role/aws-service-role/drs.amazonaws.com/AWSServiceRoleForElasticDisasterRecovery"
        ]
      },
      "Action": [
        "kms:Encrypt",
...
aws drs update-replication-configuration-template \
  ...
  --ebs-encryption-key-arn arn:aws:kms:us-east-1:664418995426:key/$KEY_ID \
  ...
  "Statement": [
    {
      ...
      "Resource": "arn:aws:kms:us-east-1:664418995426:key/*",
      "Condition": {
        "StringEquals": {
          "kms:ViaService": [
            "drs.us-west-2.amazonaws.com",
...
aws kms describe-key \
  --key-id arn:aws:kms:us-east-1:664418995426:key/$KEY_ID \
  --region us-east-1
...
aws kms describe-key \
  --key-id arn:aws:kms:us-east-1:664418995426:alias/drs-cross-account-ebs \
  --region us-east-1
...
aws drs describe-source-servers \
  --region us-west-2 \
  --filters '{"stagingAccountIDs": ["664418995426"]}' \
...
User: arn:aws:iam::160885257264:role/... is not authorized to perform: 
kms:Decrypt on resource: arn:aws:kms:us-east-1:664418995426:key/...
```
...
This setup enables:
- ✅ Staging account (664418995426) creates encrypted EBS snapshots using CMK
- ✅ Target account (160885257264) can decrypt snapshots for recovery
