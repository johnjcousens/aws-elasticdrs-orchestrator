...
│  └──────────────────────┘          └──────────────────────┘            │
│                                                                          │
│  Recovery Target: 160885257264 (via extended source servers)            │
└─────────────────────────────────────────────────────────────────────────┘
...
```json
{
  ...
  "staging_account_id": "160885257264",
  "source_region": "us-east-1",
  "target_region": "us-west-2",
  "source_role_arn": "arn:aws:iam::160885257264:role/DRSOrchestrationRole-dev",
  "staging_role_arn": "arn:aws:iam::160885257264:role/DRSOrchestrationRole-dev",
...
```json
{
  ...
  "source_region": "us-east-1",
  "target_region": "us-west-2",
  "source_role_arn": "arn:aws:iam::664418995426:role/DRSOrchestrationRole-dev",
  ...
  "extended_source_server_config": {
    "target_account_id": "160885257264",
    "shared_kms_key_arn": "arn:aws:kms:us-west-2:160885257264:key/XXXXXXXX"
  }
}
```
...
cat > kms-key-policy.json << 'EOF'
{
  ...
  "Statement": [
    ...
    },
    {
      ...
      "Principal": {
        "AWS": [
          "arn:aws:iam::664418995426:root",
          "arn:aws:iam::664418995426:role/aws-service-role/drs.amazonaws.com/AWSServiceRoleForElasticDisasterRecovery"
        ]
      },
      "Action": [
        "kms:Decrypt",
...
aws ssm send-command \
  --document-name "AWSDisasterRecovery-InstallDRAgentOnInstance" \
  --instance-ids i-XXXXXXXXX \
  --parameters "ReplicationServerRegion=us-west-2,StagingAccountID=160885257264" \
  --region us-east-1
```
...
TARGET_COUNT=$(aws drs describe-source-servers \
  --region us-west-2 \
  --query 'length(items[?!contains(arn, `664418995426`)])' \
  --output text)
