...
- **Staging Account**: 664418995426 (source instances + DRS replication staging area)
- **Target/Recovery Account**: 160885257264 (where recovery instances launch via shared EBS encryption key)
- **Orchestration Account**: 438465159935 (where Lambda runs)
...
- [x] DRS initialized in staging account (664418995426)
- [x] DRS initialized in target account (160885257264)
...
**Staging Account (664418995426) - Where source instances live**
```bash
...
**Target Account (160885257264) - Where recovery instances launch**
```bash
  ...
  "Statement": [
    {
      ...
      "Effect": "Allow",
      "Principal": {
        "AWS": "arn:aws:iam::664418995426:root"
      },
      "Action": [
        "kms:Decrypt",
        "kms:DescribeKey",
  ...
  "Statement": [
    {
      ...
      "Condition": {
        "StringEquals": {
          "sts:ExternalId": "drs-orchestration-dev-664418995426"
        }
      }
    }
  ]
...
**From Orchestration Account (438465159935)**
```bash
# Test assume role to staging account
AWS_PAGER="" aws sts assume-role \
  --role-arn arn:aws:iam::664418995426:role/DRSOrchestrationRole-dev \
  --role-session-name test-session \
  --external-id drs-orchestration-dev-664418995426

# Test assume role to target account
AWS_PAGER="" aws sts assume-role \
  --role-arn arn:aws:iam::160885257264:role/DRSOrchestrationRole-dev \
  --role-session-name test-session \
...
```bash
# Create role in staging account (664418995426)
...
```bash
# Create role in target account (160885257264)
...
cat > target-permissions-policy.json << 'EOF'
{
  ...
  "Statement": [
    ...
    },
    {
      ...
      "Resource": "arn:aws:kms:*:160885257264:key/*",
      "Condition": {
        "StringEquals": {
...
aws ec2 create-tags \
  ...
  --tags \
    ...
    Key=dr:target-account,Value=160885257264
```
...
```json
{
  ...
  "target_role_arn": "arn:aws:iam::160885257264:role/DRSOrchestrationRole-dev",
  "external_id": "drs-orchestration-dev-664418995426",
...
- [ ] DRS initialized in staging account (664418995426)
- [ ] DRS initialized in target account (160885257264)
...
- [ ] EC2 instances tagged with `dr:target-account=160885257264`
...
- [ ] Recovery instances can launch in target account (160885257264)
