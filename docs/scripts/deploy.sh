...
#
#   3. External Orchestration integration with frontend:
#      ./scripts/deploy.sh dev --orchestration-role arn:aws:iam::123456789012:role/ExternalOrchestrationRole
#      - Uses external orchestration role
#      - Deploys full frontend
#
#   4. Full External Orchestration integration (API-only):
...
# cfn-lint - skip due to performance issues
echo -e "${YELLOW}  ⚠ cfn-lint: skipped${NC}"
...
if [ "$SKIP_SECURITY" = false ]; then
    ...
    # CloudFormation Security - cfn_nag - skip
    echo -e "${YELLOW}  ⚠ cfn_nag: skipped${NC}"
...
if [ "$FRONTEND_ONLY" = false ]; then
    ...
    .venv/bin/python3 package_lambda.py > /dev/null 2>&1
