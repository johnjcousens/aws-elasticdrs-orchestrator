...
| DRS:InitiatedBy | testuser@example.com | system* | ⚠️ SEE NOTE |
...
**Problem**: The `extract_cognito_user()` function returned `{"email": "system", "userId": "system"}` instead of the actual user email `testuser@example.com`.

**Evidence**:
- Cognito claims in request: `"email": "testuser@example.com"`
...
**Environment**: drs-orchestration-test (AWS Account 438465159935)
