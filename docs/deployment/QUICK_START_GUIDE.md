...
AWS_PAGER="" aws stepfunctions start-execution \
  --state-machine-arn arn:aws:states:us-east-1:438465159935:stateMachine:DRSOrchestrationStateMachine-RMk7viQfO24W \
...
```bash
# API Handler
AWS_PAGER="" aws logs tail /aws/lambda/aws-drs-orch-api-handler-dev --follow --region us-east-1

# Orchestration
AWS_PAGER="" aws logs tail /aws/lambda/aws-drs-orch-orch-sf-dev --follow --region us-east-1
```
...
AWS_PAGER="" aws dynamodb scan \
  --table-name aws-drs-orch-protection-groups-dev \
  --region us-east-1

# Recovery Plans
AWS_PAGER="" aws dynamodb scan \
  --table-name aws-drs-orch-recovery-plans-dev \
  --region us-east-1
```
...
AWS_PAGER="" aws lambda update-function-code \
  --function-name aws-drs-orch-api-handler-dev \
  --s3-bucket aws-drs-orch-dev \
...
1. Check CloudFront distribution status: `aws cloudfront get-distribution --id E2A8KJ5LN7RQJD`
2. Verify S3 bucket has files: `aws s3 ls s3://aws-drs-orch-fe-438465159935-dev/`
