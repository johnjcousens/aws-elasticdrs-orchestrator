...
```
Developer → Build Locally → Upload Artifacts → CloudFormation Deploy
```
...
```
CloudFormation → Checkout Source → Build → Deploy
```
...
aws s3 cp orchestration.zip s3://drs-orchestration-lambda-source-438465159935/lambda/
