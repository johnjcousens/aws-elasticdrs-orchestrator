...
class TestRecoveryPlanQuotaEnforcement:
    ...
    def test_rp_total_at_limit_500_servers(self, mock_get_servers):
        ...
        result = check_total_servers_in_jobs_limit(region="us-east-1", new_server_count=500)
        assert result["valid"] is True
    ...
    def test_rp_exceeds_total_limit_501_servers(self, mock_get_servers):
        ...
        result = check_total_servers_in_jobs_limit(region="us-east-1", new_server_count=501)
        assert result["valid"] is False
        ...
        active_jobs = [
            {
                ...
                "status": "STARTED",
                "participatingServers": [{"sourceServerID": f"s-{j:019d}"} for j in range(10)],
            }
            for i in range(20)
        ]
...
class TestTotalServersInJobsQuotaEnforcement:
    ...
    def test_total_servers_at_limit_500_servers(self, mock_get_servers):
        ...
        result = check_total_servers_in_jobs_limit(region="us-east-1", new_server_count=1)
    ...
    def test_total_servers_below_limit_400_servers(self, mock_get_servers):
        ...
        result = check_total_servers_in_jobs_limit(region="us-east-1", new_server_count=50)
...
class TestServerConflictDetection:
    ...
    def test_server_already_in_job_conflict(self, mock_drs_jobs, mock_executions):
        ...
        mock_drs_jobs.return_value = {"s-0000000000000000001": {"jobId": "drsjob-001", "jobStatus": "STARTED"}}

        # Create plan with same server
        plan = {"planId": "test-plan", "waves": [{"waveName": "Wave-1", "protectionGroupId": "pg-test"}]}
        ...
        with patch("shared.conflict_detection.resolve_pg_servers_for_conflict_check") as mock_resolve:
            ...
            with patch("shared.conflict_detection.protection_groups_table") as mock_table:
                mock_table.get_item.return_value = {"Item": {"region": "us-east-1"}}
    ...
    def test_no_server_conflicts(self, mock_drs_jobs, mock_executions):
        ...
        mock_drs_jobs.return_value = {f"s-{i:019d}": {"jobId": "drsjob-001", "status": "STARTED"} for i in range(1, 11)}
...
class TestMultiAccountCapacityTracking:
        ...
        replicating_servers = [
            {
                ...
                "stagingArea": {"stagingAccountID": "123456789012"},
            }
            for i in range(300)
        ]

        mock_drs.describe_source_servers.return_value = {"items": replicating_servers}

        # Count replicating servers
        response = mock_drs.describe_source_servers()
        replicating_count = len([s for s in response["items"] if s.get("replicationStatus") == "REPLICATING"])

        assert replicating_count == 300
        # At hard limit - should not allow more

    @patch("shared.conflict_detection.create_drs_client")
    def test_extended_source_servers_not_counted_in_replicating(self, mock_create_client):
            ...
            servers.append(
                {
                    "sourceServerID": f"s-{i:019d}",
                    "replicationStatus": "REPLICATING",
                    "stagingArea": {"stagingAccountID": "123456789012"},  # Target account
                }
            )
            ...
            servers.append(
                {
                    "sourceServerID": f"s-{i:019d}",
                    "replicationStatus": "REPLICATING",
                    "stagingArea": {"stagingAccountID": "999999999999"},  # Staging account
                }
            )
        ...
        replicating_in_target = len(
            [
                ...
                if (
                    s.get("replicationStatus") == "REPLICATING"
                    and s.get("stagingArea", {}).get("stagingAccountID") == target_account
                )
            ]
        )
    ...
    @patch("shared.conflict_detection.create_drs_client")
    def test_multi_account_capacity_4000_source_servers(self, mock_create_client):
                ...
                servers.append(
                    {
                        ...
                        "replicationStatus": "REPLICATING",
                        "stagingArea": {"stagingAccountID": f"99999999{account_idx:04d}"},
                    }
                )
            ...
            servers.append(
                {
                    "sourceServerID": server_id,
                    "replicationStatus": "REPLICATING",
                    "stagingArea": {"stagingAccountID": "999999999013"},
                }
            )
...
class TestComprehensiveQuotaValidation:
    ...
    def test_all_quotas_at_maximum_capacity(self, mock_get_servers, mock_create_client):
            ...
            active_jobs.append(
                {
                    "jobID": f"drsjob-{i:03d}",
                    "status": "STARTED",
                    "participatingServers": [{"sourceServerID": f"s-{j:019d}"} for j in range(i * 25, (i + 1) * 25)],
                }
            )
        ...
        total_result = check_total_servers_in_jobs_limit(region="us-east-1", new_server_count=1)
    ...
    def test_quota_validation_provides_recommendations(self, mock_get_servers):
        ...
        result = check_total_servers_in_jobs_limit(region="us-east-1", new_server_count=600)
...
class TestQuotaDocumentationAlignment:
    ...
    def test_max_servers_per_job_matches_docs(self, mock_resolve):
        ...
        # Test over limit
        servers_over_limit = [f"s-{i:019d}" for i in range(DOCUMENTED_LIMIT + 1)]
    ...
    def test_max_total_servers_matches_docs(self, mock_get_servers):
        ...
        # Test at limit
        result = check_total_servers_in_jobs_limit(region="us-east-1", new_server_count=DOCUMENTED_LIMIT)
        assert result["valid"] is True
        assert result["totalAfter"] == DOCUMENTED_LIMIT

        # Test over limit
        result = check_total_servers_in_jobs_limit(region="us-east-1", new_server_count=DOCUMENTED_LIMIT + 1)
        assert result["valid"] is False
        ...
        active_jobs = [
            {
                "jobID": f"drsjob-{i:03d}",
                "status": "STARTED",
                "participatingServers": [{"sourceServerID": f"s-{i:019d}"}],
            }
