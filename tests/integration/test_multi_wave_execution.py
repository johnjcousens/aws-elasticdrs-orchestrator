...
def mock_drs_job_response(job_id, status="COMPLETED", server_count=2):
    ...
    return {"items": [{"jobID": job_id, "status": status, "type": "LAUNCH", "participatingServers": servers}]}
...
class TestMultiWaveExecution:
    ...
    def test_three_wave_execution_complete_flow(self, dynamodb_table, mock_env_vars):
        """
        ...
        execution = create_multi_wave_execution(dynamodb_table, execution_id="exec-3wave", plan_id="plan-1")
        ...
        with patch("boto3.client") as mock_boto_client:
            ...
            with patch("shared.drs_utils.enrich_server_data") as mock_enrich:
                ...
                poll_event = {"operation": "poll", "executionId": "exec-3wave", "planId": "plan-1"}
            ...
            response = dynamodb_table.get_item(Key={"executionId": "exec-3wave", "planId": "plan-1"})
        ...
        finalize_event = {"operation": "finalize", "executionId": "exec-3wave", "planId": "plan-1"}
    ...
    def test_execution_status_never_changes_during_polling(self, dynamodb_table, mock_env_vars):
        """
        ...
        execution = create_multi_wave_execution(dynamodb_table, execution_id="exec-status-check", plan_id="plan-1")
        ...
        with patch("boto3.client") as mock_boto_client:
            ...
            with patch("shared.drs_utils.enrich_server_data") as mock_enrich:
                ...
                poll_event = {"operation": "poll", "executionId": "exec-status-check", "planId": "plan-1"}
                ...
                response = dynamodb_table.get_item(Key={"executionId": "exec-status-check", "planId": "plan-1"})
                assert response["Item"]["status"] == "POLLING"

    def test_cannot_finalize_with_incomplete_waves(self, dynamodb_table, mock_env_vars):
        """
        ...
        execution = create_multi_wave_execution(dynamodb_table, execution_id="exec-incomplete-waves", plan_id="plan-1")

        # Update wave 0 to COMPLETED, leave waves 1-2 PENDING
        response = dynamodb_table.get_item(Key={"executionId": "exec-incomplete-waves", "planId": "plan-1"})
        exec_item = response["Item"]
        exec_item["waves"][0]["status"] = "COMPLETED"
        dynamodb_table.put_item(Item=exec_item)

        # Try to finalize
        finalize_event = {"operation": "finalize", "executionId": "exec-incomplete-waves", "planId": "plan-1"}
    ...
    def test_each_wave_enriched_independently(self, dynamodb_table, mock_env_vars):
        """
        ...
        # Create 3-wave execution
        execution = create_multi_wave_execution(dynamodb_table, execution_id="exec-enrichment", plan_id="plan-1")
        ...
        with patch("boto3.client") as mock_boto_client:
            ...
            response = dynamodb_table.get_item(Key={"executionId": "exec-enrichment", "planId": "plan-1"})
            exec_item = response["Item"]
            exec_item["waves"][1]["status"] = "IN_PROGRESS"
            dynamodb_table.put_item(Item=exec_item)
    ...
    def test_all_waves_complete_flag_accuracy(self, dynamodb_table, mock_env_vars):
        """
        ...
        # Create 3-wave execution
        execution = create_multi_wave_execution(dynamodb_table, execution_id="exec-flag-test", plan_id="plan-1")
        ...
        with patch("boto3.client") as mock_boto_client:
            ...
            with patch("shared.drs_utils.enrich_server_data") as mock_enrich:
                ...
                poll_event = {"operation": "poll", "executionId": "exec-flag-test", "planId": "plan-1"}
                ...
                # Complete wave 1
                response = dynamodb_table.get_item(Key={"executionId": "exec-flag-test", "planId": "plan-1"})
                exec_item = response["Item"]
    ...
    def test_pause_resume_between_waves(self, dynamodb_table, mock_env_vars):
        """
        ...
        response = dynamodb_table.get_item(Key={"executionId": "exec-pause-resume", "planId": "plan-1"})
        assert response["Item"]["status"] == "PAUSED"

        # Step 2: Resume execution via operation-based invocation
        resume_event = {"operation": "resume", "executionId": "exec-pause-resume"}
    ...
    def test_execution_finalization_updates_all_fields(self, dynamodb_table, mock_env_vars):
        """
        ...
        # Finalize execution
        finalize_event = {"operation": "finalize", "executionId": "exec-finalize-fields", "planId": "plan-1"}
        ...
        response = dynamodb_table.get_item(Key={"executionId": "exec-finalize-fields", "planId": "plan-1"})
    ...
    def test_server_data_enrichment_preserves_drs_fields(self, dynamodb_table, mock_env_vars):
        """
        ...
        execution = create_multi_wave_execution(dynamodb_table, execution_id="exec-enrichment-fields", plan_id="plan-1")
        ...
        with patch("boto3.client") as mock_boto_client:
            ...
            with patch("shared.drs_utils.enrich_server_data") as mock_enrich:
                ...
                poll_event = {"operation": "poll", "executionId": "exec-enrichment-fields", "planId": "plan-1"}
        ...
        response = dynamodb_table.get_item(Key={"executionId": "exec-enrichment-fields", "planId": "plan-1"})
