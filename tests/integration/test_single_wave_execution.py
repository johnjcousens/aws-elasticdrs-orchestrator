...
class TestSingleWaveExecution:
    ...
    def test_single_wave_execution_complete_flow(
        ...
        execution = create_single_wave_execution(
            dynamodb_table,
            execution_id="exec-single-1",
            plan_id="plan-1"
        )
        ...
        response = dynamodb_table.get_item(
            Key={"executionId": "exec-single-1", "planId": "plan-1"}
        )
    ...
    def test_polling_never_changes_execution_status(
        self, dynamodb_table, mock_env_vars, mock_drs_api
    ):
        """
        ...
        response = dynamodb_table.get_item(
            Key={"executionId": "exec-status-test", "planId": "plan-1"}
        )
        
        assert response["Item"]["status"] == "POLLING"

    def test_finalization_requires_all_waves_complete(
        self, dynamodb_table, mock_env_vars
    ):
        """
        ...
        response = dynamodb_table.get_item(
            Key={"executionId": "exec-incomplete", "planId": "plan-1"}
        )
        assert response["Item"]["status"] == "POLLING"
    ...
    def test_server_data_enrichment_with_ec2_details(
        self, dynamodb_table, mock_env_vars
    ):
        """
