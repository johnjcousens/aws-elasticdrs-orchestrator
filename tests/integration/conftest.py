...
def pytest_configure(config):
    ...
    os.environ["AWS_PROFILE"] = "210987654321_AdministratorAccess"
    
    # Force boto3 to reload credentials
    boto3.setup_default_session(profile_name="210987654321_AdministratorAccess")
...
def aws_credentials():
    ...
    try:
        ...
        assert account_id == "210987654321", (
            f"Wrong AWS account: {account_id}. "
            f"Expected QA orchestration account 210987654321. "
        ...
        pytest.fail(
            ...
            f"  Error: {e}\n"
            f"  Please authenticate with: aws login\n"
            f"  Then set profile: export AWS_PROFILE=210987654321_AdministratorAccess"
        )
