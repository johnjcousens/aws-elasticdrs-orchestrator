...
os.environ["EXECUTION_NOTIFICATIONS_TOPIC_ARN"] = "arn:aws:sns:us-east-1:123456789012:test-notifications"
...
SUB_ARN = "arn:aws:sns:us-east-1:123456789012:" "test-notifications:sub-uuid-1234"
NEW_SUB_ARN = "arn:aws:sns:us-east-1:123456789012:" "test-notifications:sub-uuid-5678"
        ...
        "requestContext": {
            ...
            "identity": {
                ...
                "cognitoAuthenticationProvider": (
                    ...
                    ".com/pool:CognitoSignIn:" + VALID_ACCOUNT
                ),
            },
        },
...
class TestPGCreationAccountContext:
    ...
    def test_api_gw_with_valid_account(
        ...
        mock_drs = MagicMock()
        mock_drs.describe_source_servers.return_value = {"items": [{"sourceServerID": "s-12345678901234567"}]}
        body = {
            "groupName": "TestPG",
            "region": "us-east-1",
        ...
        with (
            _patch_tables(pg_table, rp_table, exec_table),
            patch.object(
                handler_mod,
                "create_drs_client",
            ...
            result = handler_mod.create_protection_group(event, body)
    ...
    def test_direct_invocation_with_valid_account(
        ...
        event = _direct_event("create_protection_group", body)
        with (
            _patch_tables(pg_table, rp_table, exec_table),
            patch.object(
                handler_mod,
    ...
    def test_direct_invocation_missing_account_fails(
        ...
        event = _direct_event("create_protection_group", body)
        with _patch_tables(pg_table, rp_table, exec_table):
            result = handler_mod.create_protection_group(event, body)
...
class TestRPUpdateEmailChange:
    ...
    def test_update_email_deletes_old_creates_new(
        ...
        existing = _existing_plan(email=VALID_EMAIL, sub_arn=SUB_ARN)
        ...
        with (
            ...
            result = handler_mod.update_recovery_plan("rp-111", body)
        assert result["statusCode"] == 200
        calls = ms.call_args_list
        del_calls = [c for c in calls if c[1].get("action") == "delete"]
        cre_calls = [c for c in calls if c[1].get("action") == "create"]
...
class TestRPDeletionSubscriptionCleanup:
    ...
    def test_delete_plan_cleans_up_subscription(
        ...
        with (
            ...
            result = handler_mod.delete_recovery_plan("rp-del")
        assert result["statusCode"] == 200
        ...
        rp_table.delete_item.assert_called_once_with(Key={"planId": "rp-del"})
    ...
    def test_delete_plan_without_email_skips_cleanup(
        ...
        with (
            ...
            result = handler_mod.delete_recovery_plan("rp-noemail")
        assert result["statusCode"] == 200
        ms.assert_not_called()
