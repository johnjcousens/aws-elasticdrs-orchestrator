...
def test_property_uninitialized_region_handling(region_status):
    ...
    def mock_count_drs_servers(drs_client):
    ...
    expected_total = sum(
        count for initialized, count in region_status.values()
        if initialized
    )
    ...
    assert result["totalServers"] == expected_total, (
    ...
    for region_data in result["regionalBreakdown"]:
        ...
        if region in region_status:
            ...
            if initialized:
                ...
                assert region_data["totalServers"] == expected_count, (
...
def test_property_mixed_region_initialization(
    ...
    server_counts = {
        region: 50 for region in initialized_regions
    }
...
def test_edge_case_alternating_initialization():
    ...
    initialized_regions = {DRS_REGIONS[i] for i in range(0, len(DRS_REGIONS), 2)}
