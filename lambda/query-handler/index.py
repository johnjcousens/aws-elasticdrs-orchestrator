...
def handle_get_source_server_inventory(query_params: Dict) -> Dict:
    """
    Query source server inventory from DynamoDB inventory table.

    This endpoint queries the cached inventory database instead of making
    ...
    Returns:
        Response with list of servers from inventory database
    """
    ...
    try:
        ...
        print(f"Error querying source server inventory: {e}")
        import traceback
        ...
        return response(
            ...
            error_response(
                ERROR_INTERNAL_ERROR,
                "Failed to query source server inventory",
                details={"error": str(e)},
            ),
        )
