...
def validate_iam_authorization(principal_arn: str) -> bool:
    ...
    logger.warning(
        f"Authorization denied for principal: {principal_arn}. " "Principal does not match allowed patterns."
    )
    return False
...
def create_authorization_error_response() -> Dict[str, Any]:
    ...
    return {
        "error": "AUTHORIZATION_FAILED",
        "message": "Insufficient permissions for direct invocation",
        "details": ("Direct Lambda invocation requires OrchestrationRole or " "equivalent IAM permissions"),
    }
