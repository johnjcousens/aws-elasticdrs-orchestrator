...
This Lambda implements the DRS adapter for the Enterprise DR Orchestration Platform (External).
...
Lambda invocation (future External Orchestration integration mode). In External mode, this function is called
directly by the External orchestration Step Functions without API Gateway or Cognito,
...
- In External mode, authentication handled by External platform (no Cognito)
...
blueprint for other adapters to follow in the broader External platform.
...
#
# External ADAPTER PATTERN NOTE:
# This Step Functions Lambda currently reads Protection Groups and Recovery Plans
# directly from DynamoDB. In the broader External platform, this pattern will be replicated
...
#
# Future External Orchestration Integration:
# When integrated into the External platform, this DRS adapter will be invoked by the
# External orchestration Step Functions, which will pass Protection Group and Recovery Plan
