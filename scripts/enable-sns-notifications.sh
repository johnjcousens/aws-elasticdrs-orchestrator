...
if [ -n "$TOPIC_ARN" ]; then
  ...
  # Check subscription
  SUBSCRIPTION=$(aws sns list-subscriptions-by-topic --topic-arn "$TOPIC_ARN" --region "$REGION" 2>/dev/null | grep -o "***REMOVED***" || echo "")
  
  if [ -n "$SUBSCRIPTION" ]; then
    echo "✅ Email subscription confirmed for: ***REMOVED***"
    echo ""
    ...
    echo "Check your email (***REMOVED***) for a confirmation message"
  fi
else
