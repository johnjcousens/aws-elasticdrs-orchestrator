...
create_test_user() {
    ...
    if [[ "$DRY_RUN" == "true" ]]; then
        log_info "[DRY RUN] Would create test user: ***REMOVED***"
        return 0
    fi
    ...
    local test_email="***REMOVED***"
    local test_password="***REMOVED***"
...
main() {
    ...
    echo "   2. Login with admin email: ***REMOVED*** (check email for temp password)"
