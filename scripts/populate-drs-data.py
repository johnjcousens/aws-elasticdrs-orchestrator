...
# Configuration
EXECUTION_HISTORY_TABLE = "aws-drs-orch-execution-history-dev"
...
def enrich_execution_with_drs_data(
    execution: Dict, recovery_instances: Dict[str, Dict]
) -> Dict:
    ...
    for wave_idx, wave in enumerate(waves):
        ...
        if job_id and not servers and not server_statuses and server_ids:
            ...
            try:
                ...
                if response.get("items"):
                    ...
                    print(
                        f"    ✅ Found DRS job: status={drs_status}, servers={len(participating_servers)}"
                    )
                    ...
                    for drs_server in participating_servers:
                        ...
                        launch_status = drs_server.get(
                            "launchStatus", "UNKNOWN"
                        )
                        recovery_info = recovery_instances.get(
                            source_server_id, {}
                        )
                        ...
                        instance_id = (
                            ...
                            or recovery_info.get("recoveryInstanceID")
                            or recovery_info.get("ec2InstanceID")
                        )
                        ...
                        if instance_id:
                            ...
                            if ec2_details:
                                server_data["HostName"] = ec2_details.get(
                                    "hostname", ""
                                )
                                server_data["PrivateIpAddress"] = (
                                    ec2_details.get("privateIpAddress", "")
                                )
                                server_data["InstanceType"] = ec2_details.get(
                                    "instanceType", ""
                                )

                                if ec2_details.get("launchTime"):
                                    server_data["LaunchTime"] = int(
                                        ec2_details["launchTime"].timestamp()
                                    )

                                print(
                                    f"      ✅ Enriched {source_server_id}: {instance_id} ({ec2_details.get('instanceType', 'unknown')})"
                                )
                                enriched_servers += 1
        ...
        # Enrich existing servers with DRS data if missing
            ...
            for server_status in server_statuses:
                ...
                # Get instance ID from multiple sources
                instance_id = (
                    server_status.get("RecoveryInstanceID")
                    or recovery_info.get("recoveryInstanceID")
                    or recovery_info.get("ec2InstanceID")
                )
                ...
                if instance_id and not server_status.get("RecoveryInstanceID"):
                    print(
                        f"      ✅ Found instance ID for {source_server_id}: {instance_id}"
                    )
                ...
                if recovery_info:
                    server_status["InstanceType"] = recovery_info.get(
                        "ec2InstanceType", ""
                    )
                    server_status["JobID"] = recovery_info.get("jobID", job_id)
                    server_status["PointInTimeDateTime"] = recovery_info.get(
                        "pointInTimeDateTime", ""
                    )
            ...
            for server in servers:
                ...
                if instance_id:
                    ...
                    ec2_details = get_ec2_instance_details(instance_id)
                    if ec2_details:
                        server["HostName"] = ec2_details.get(
                            "hostname", server.get("HostName", "")
                        )
                        server["PrivateIpAddress"] = ec2_details.get(
                            "privateIpAddress",
                            server.get("PrivateIpAddress", ""),
                        )
    ...
    print(
        f"🔧 Enriched {enriched_servers} servers in execution {execution_id}"
    )
...
def update_execution_in_dynamodb(execution: Dict) -> bool:
    ...
    try:
        ...
        dynamodb.update_item(
            ...
            ExpressionAttributeValues={":waves": {"L": waves_dynamodb}},
        )
...
def main():
    ...
    for execution in executions:
        ...
        enriched_execution = enrich_execution_with_drs_data(
            execution, recovery_instances
        )
