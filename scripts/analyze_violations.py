...
class ViolationAnalyzer:
    ...
    def get_priority_files(self) -> List[Tuple[str, int, str]]:
        ...
        priority_weights = {"lambda/": 100, "scripts/": 50, "tests/": 10}
        ...
        for file_path, analysis in file_analysis.items():
            ...
            file_priorities.append(
                (file_path, priority_score, self._get_file_type(file_path))
            )
    ...
    def generate_migration_plan(self) -> Dict:
        ...
        for file_path, analysis in file_analysis.items():
            ...
            if critical_count > 0 or complexity_count > 5:
                ...
                analysis["by_category"]["whitespace"]
                > analysis["total_violations"] * 0.7
            ):
        ...
        return {
            "summary": {
                ...
                "by_category": {
                    category: sum(
                        1 for v in self.violations if v["category"] == category
                    )
            ...
            "recommendations": self._generate_recommendations(
                file_analysis, type_analysis
            ),
        }

    def _generate_recommendations(
        self, file_analysis: Dict, type_analysis: Dict
    ) -> List[str]:
        ...
        lambda_files = [
            f for f in file_analysis.keys() if f.startswith("lambda/")
        ]
        if lambda_files:
            recommendations.append(
    ...
    def save_analysis(
        self, output_file: str = "violation_analysis.json"
    ) -> None:
    ...
    def print_summary(self) -> None:
        ...
        print(
            f"  Complex refactor: {len(categories['complex_refactor'])} files"
        )
...
def main():
    ...
    if not violation_lines or (
        len(violation_lines) == 1 and not violation_lines[0]
    ):
