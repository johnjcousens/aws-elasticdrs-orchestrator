...
if [[ -n "$USER_POOL_ID" ]]; then
    ...
    # Check for test user
    if aws cognito-idp admin-get-user --user-pool-id "$USER_POOL_ID" --username "testuser@example.com" --region "$AWS_REGION" > /dev/null 2>&1; then
...
if [[ $FAILED_CHECKS -eq 0 ]]; then
    ...
    echo "   2. Login with test user: testuser@example.com / TestPassword123!"
