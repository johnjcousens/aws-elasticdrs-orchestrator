...
if [ -f "$API_HANDLER" ]; then
    ...
    if grep -n "\"GroupName\"" "$API_HANDLER" | grep -v "sg\[" | grep -v "result\[" | grep -v "ec2\\.describe" | grep -v "in existing_group:" | grep -v "REMOVE" | grep -v "remove_clauses.append" > /dev/null 2>&1; then
