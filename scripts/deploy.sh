...
if ! aws sts get-caller-identity > /dev/null 2>&1; then
    ...
    if echo "$ERROR_MSG" | grep -q "SSO session"; then
        ...
        echo -e "${YELLOW}   Run: aws sso login --profile ${AWS_PROFILE:-210987654321_AdministratorAccess}${NC}"
    else
        ...
        echo -e "${YELLOW}   Example: export AWS_PROFILE=210987654321_AdministratorAccess${NC}"
    fi
    exit 1
fi
