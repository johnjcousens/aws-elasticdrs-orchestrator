...
def count_safety_issues(safety_report):
    ...
    if isinstance(safety_report, list):
        ...
        isinstance(safety_report, dict) and "vulnerabilities" in safety_report
    ):
...
def generate_summary():
    ...
    cfn_lint_count = (
        len(cfn_lint) if cfn_lint and isinstance(cfn_lint, list) else 0
    )
    ...
    summary = {
        ...
        "results": {
            ...
            "frontend_security": {"npm_vulnerabilities": npm_counts},
    ...
    print(
        f"  Bandit Issues: {bandit_counts['total']} (High: {bandit_counts['high']}, Medium: {bandit_counts['medium']}, Low: {bandit_counts['low']})"
    )
    print(
        f"  Semgrep Findings: {semgrep_python_counts['total']} (Error: {semgrep_python_counts['error']}, Warning: {semgrep_python_counts['warning']})"
    )
    print(f"  Vulnerable Dependencies: {safety_count}")

    print("\n🌐 Frontend Security:")
    print(
        f"  NPM Vulnerabilities: {npm_counts['total']} (Critical: {npm_counts['critical']}, High: {npm_counts['high']}, Moderate: {npm_counts['moderate']})"
    )

    print("\n☁️ Infrastructure Security:")
    print(f"  CloudFormation Issues: {cfn_lint_count}")
    print(
        f"  Semgrep Findings: {semgrep_cfn_counts['total']} (Error: {semgrep_cfn_counts['error']}, Warning: {semgrep_cfn_counts['warning']})"
    )
    ...
    if critical_issues > CRITICAL_THRESHOLD:
        ...
        print(
            "   Critical issues block deployment and must be resolved immediately"
        )
        return 1
        ...
        print(
            "   Recommendation: Consider addressing medium/low severity issues"
        )
        return 0
    else:
        ...
        if total_issues > 0:
            print(
                f"   Note: {total_issues} low-severity issues found - consider addressing during maintenance"
            )
        return 0
...
if __name__ == "__main__":
    try:
        ...
        print(
            "Check that security scan reports exist in reports/security/raw/"
        )
        sys.exit(1)
