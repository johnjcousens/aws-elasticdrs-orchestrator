...
#
# Deploy DRS Agents to External Instances
#
# This script handles the split deployment strategy for External environment:
...
CROSS_ACCOUNT_TARGET="123456789012"  # 03/04 replicate to staging account
REGION="us-east-1"
S3_BUCKET="drs-agent-deployment-160885257264"  # Bucket in source account for scripts

# External Instance IDs - 01/02 (same-account replication)
...
# External Instance IDs - 03/04 (cross-account replication)
...
echo -e "${CYAN}=== External DRS Agent Deployment ===${NC}"
...
case "${1:-all}" in
    ...
    *)
        ...
        echo ""
        echo "  ${YELLOW}Cross-Account Replication (03/04 → 123456789012):${NC}"
