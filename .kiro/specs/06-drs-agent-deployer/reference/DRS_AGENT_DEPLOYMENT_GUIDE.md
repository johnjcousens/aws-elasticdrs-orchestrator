...
```json
{
  ...
  "role_arn": "arn:aws:iam::160885257264:role/aws-drs-orchestration-cross-account-role",
...
```bash
aws lambda invoke \
  --function-name aws-drs-orchestration-drs-agent-deployer-dev \
...
response = lambda_client.invoke(
    FunctionName='aws-drs-orchestration-drs-agent-deployer-dev',
...
def deploy_agents_to_account(account_id, source_region, target_region):
    ...
    payload = {
        ...
        'role_arn': f'arn:aws:iam::{account_id}:role/aws-drs-orchestration-cross-account-role',
...
Resources:
  DRSOrchestrationRole:
    Type: AWS::IAM::Role
    Properties:
      RoleName: aws-drs-orchestration-cross-account-role
...
```bash
# Tail logs
aws logs tail /aws/lambda/aws-drs-orchestration-drs-agent-deployer-dev --follow

# Search for errors
aws logs filter-log-events \
  --log-group-name /aws/lambda/aws-drs-orchestration-drs-agent-deployer-dev \
...
aws lambda invoke \
  --function-name aws-drs-orchestration-drs-agent-deployer-dev \
  --payload '{"account_id":"123456789012","source_region":"us-east-1","target_region":"us-west-2"}' \
  response.json
```
