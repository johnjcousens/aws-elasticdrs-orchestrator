...
### In Target/Staging Account (123456789012)
...
2. Permissions to:
   - Upload to S3 bucket (aws-drs-orchestration-dev)
...
Deploy to all External instances:
```bash
...
4. Run:
```powershell
.\install-drs-agent-cross-account.ps1 -AccountId 123456789012
```
...
Region: us-east-1
Target Account: 123456789012 (Cross-Account Replication)
...
Installer arguments: --region us-east-1 --account-id 123456789012 --no-prompt
...
- `--s3-bucket` - S3 bucket for script storage (default: aws-drs-orchestration-dev)
- `--dry-run` - Preview without executing
...
Instance IDs:
  - i-00c5c7b3cf6d8abeb
  - i-04d81abd203126050
Cross-Account Replication: Enabled (Target: 123456789012)

[1/4] Uploading PowerShell script to S3...
✓ Uploaded to: s3://aws-drs-orchestration-dev/scripts/drs/install-drs-agent-cross-account.ps1
