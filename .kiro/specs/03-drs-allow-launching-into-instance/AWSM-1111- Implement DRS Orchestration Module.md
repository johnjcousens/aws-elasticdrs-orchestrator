...
**Wave-Based Orchestration**: https://code.aws.dev/personal_projects/alias_j/jocousen/aws-elasticdrs-orchestrator/blob/main/archive/docs/Customer/Cloud%20Migration/Disaster%20Recovery/7-Implementation%20%26%20Runbooks/Enterprise%20DR%20Orchestration%20Platform%20Design%201.0%20--/AWSM-1103-Enterprise-Wave-Orchestration.md
...
### External Implementation Patterns (Adapted from AWS Labs)

**Reference**: AWS DRS Settings Tool provides production-tested patterns adapted for External's manifest-driven, Module Factory architecture.

**Key Patterns Adapted for External**:

1. **Module Factory Adapter Pattern** (Replaces Object-Oriented Settings Classes):


```py
# External Pattern: DRS adapter following Module Factory with 4-phase lifecycle
...
```py
# External Pattern: Manifest validation upstream, adapter receives clean data
class DRSAdapter(TechnologyAdapter):
...
```py
# External Pattern: DynamoDB-based state management for idempotency
class DRSAdapter(TechnologyAdapter):
...
```py
# External Pattern: STS AssumeRole with IAM cross-account roles
class DRSAdapter(TechnologyAdapter):
...
```py
# External Pattern: Wave-based with Step Functions + parallel Lambda execution
class DRSAdapter(TechnologyAdapter):
...
```py
# External Pattern: Tag-driven validation during discovery phase
class DRSAdapter(TechnologyAdapter):
...
### External vs AWS Labs Pattern Comparison

| Pattern | AWS Labs (CSV-driven) | External (Manifest-driven) |
| --- | --- | --- |
