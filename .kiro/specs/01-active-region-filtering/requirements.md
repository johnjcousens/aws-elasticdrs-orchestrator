...
**Context:** The platform maintains a `SOURCE_SERVER_INVENTORY_TABLE` (aws-drs-orchestration-source-server-inventory-dev) that is synced every 15 minutes from DRS and EC2 APIs. This table contains comprehensive server information including hardware specs, network configuration, tags, and replication state.
