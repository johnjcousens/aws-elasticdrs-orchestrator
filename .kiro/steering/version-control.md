...
## Purpose
This document defines commit message standards using Conventional Commits to ensure consistent, traceable, and professional version control practices across all projects.
...
feat(drs): add source server recovery endpoint
fix(lambda): resolve DRS job status polling timeout
```
