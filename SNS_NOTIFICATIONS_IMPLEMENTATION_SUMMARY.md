...
AdminEmail: 'jocousen@amazon.com'  # Still required but unused
```
...
aws cloudformation deploy --template-file cfn/master-template.yaml \
  --parameter-overrides AdminEmail=jocousen@amazon.com

# Deploy with notifications disabled
aws cloudformation deploy --template-file cfn/master-template.yaml \
  --parameter-overrides AdminEmail=jocousen@amazon.com EnablePipelineNotifications=false
```
